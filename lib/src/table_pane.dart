    ...
    ColoredBox(
      ...
      child: Directionality(
        ...
        child: DefaultTextStyle(
          style: const TextStyle(fontFamily: 'Verdana', color: Color(0xff000000)),
          child: Row(
              ...
              IntrinsicWidth(
                child: IntrinsicHeight(
                  child: TablePane(
                    ...
                    children: <Widget>[
                      TableRow(
                        height: const RelativeTablePaneRowHeight(weight: 5),
                        children: const [
                          ColoredCell(),
                          ColoredCell(),
                          ColoredCell(),
...
class ColoredCell extends StatelessWidget {
  const ColoredCell({this.text = 'cell', super.key});
...
class TablePaneElement extends MultiChildRenderObjectElement {
  TablePaneElement(TablePane super.widget);
}
...
class TableRowElement extends MultiChildRenderObjectElement {
  TableRowElement(TableRow super.widget);
}
...
mixin ChildListRenderObjectMixin<ChildType extends RenderBox, ParentDataType extends ContainerBoxParentData<ChildType>>
...
class RenderTablePane extends RenderBox
  ...
  List<double> _computeActualRowHeights(
    ...
    if (heightConstraints.isSatisfiedBy(totalHeight)) {
      if (verticalRelativeSize == MainAxisSize.max && heightConstraints.isBounded && totalRelativeWeight > 0) {
    ...
    assert(() {
      if (rowHeights.any((double value) => value.isNegative)) {
        throw FlutterError.fromParts(<DiagnosticsNode>[
          ...
          ErrorSpacer(),
          DiagnosticsProperty<Object>('The RenderTablePane in question was created by', debugCreator,
          ...
          if (columnSpan > 1) {
            ...
            if (spannedRelativeWeight > 0 || spannedIntrinsicWidthCellCount > 0) {
              ...
              if (isRelativeOrIntrinsic(heightSpec)) {
                ...
                final int rowSpan = childParentData.rowSpan;
                final Iterable<TablePaneRowHeight> spannedRowHeights =
                    rows.map<TablePaneRowHeight>((RenderTableRow row) => row.height).skip(i).take(rowSpan);
                if (spannedRowHeights.any(isRelativeOrIntrinsic)) {
                  heightConstraint = double.infinity;
                } else {
                  heightConstraint =
                      spannedRowHeights.map<double>((TablePaneRowHeight spec) => spec.height).fold<double>(0, _sum);
  ...
  List<double> _computeActualColumnWidths(LinearConstraints widthConstraints) {
    ...
    if (widthConstraints.isSatisfiedBy(totalWidth)) {
      if (horizontalRelativeSize == MainAxisSize.max && widthConstraints.isBounded && totalRelativeWeight > 0) {
      ...
      if (rows.isNotEmpty) {
        ...
        if (cellsPerRow != columns.length) {
          throw FlutterError.fromParts(<DiagnosticsNode>[
            ...
            ErrorSpacer(),
            IntProperty('The number of cells in each row was', cellsPerRow, style: DiagnosticsTreeStyle.errorProperty),
            IntProperty('The number of columns was', columns.length, style: DiagnosticsTreeStyle.errorProperty),
            ErrorSpacer(),
            DiagnosticsProperty<Object>('The RenderTablePane in question was created by', debugCreator,
                style: DiagnosticsTreeStyle.errorProperty),
          ]);
