...
mixin RenderTableViewMixin on RenderSegment {
  ...
  void rebuildIfNecessary() {
    ...
    if (_needsBuild) {
      removeCells = UnionTableCellRange(
          <TableCellRange>[removeCells, builtCells.where((int rowIndex, int columnIndex) => rowIndex >= length)]);
    ...
    if (maxWidthDelta.isNegative) {
      ...
      if (flexAllocation > 0) {
        for (MapEntry<int, AbstractTableColumn> flexColumn in flexColumns.entries) {
          final FlexTableColumnWidth widthSpecification = flexColumn.value.width as FlexTableColumnWidth;
